...
int main(int argc, char* argv[])
{
	...
	ofstream X;
	X.open("result1/x"+to_string(SIZEE)+".txt");
	...
	for (int k = 0; k < NoOfLines; k++) 
	{
		...
		*/
		...
		cout<<"1-Blockage= "<<100-blockage<<endl;
		Graph g(n);
		...
		ofstream Y;
		Y.open("result1/fracy" + to_string(k) + ".txt");
	...
	ofstream Y;
	Y.open("result1/fracyavg"+to_string(SIZEE)+".txt");
	for (int i = 0; i < fracyavg.size(); i++)
		Y << fracyavg[i] /threshold<< " ";
	Y << endl;
	Y.close();
	Y.open("result1/flowyavg"+to_string(SIZEE)+".txt");
